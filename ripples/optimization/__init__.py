...
3. `_steepest_descent_optimization.py`
    First-order methods that do not need a line search: Nesterov accelerated
    gradient descent and Adam. It is the simplest family of methods and serves
    as a reference for the more sophisticated algorithms that follow.

4. `_line_search_utils.py`
    Strong-Wolfe line search (bracketing + zoom phase) with full parameter
    validation. Used by every method in in `_line_search_optimization.py`.

5. `_line_search_optimization.py`
    Methods that use the line search as their step-length calculator nonlinear
    conjugate gradient (Polak-Ribière+), BFGS, L-BFGS, and the Newton-CG
    variant.

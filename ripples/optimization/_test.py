...
This script exercises every public surface of the optimization module
(***minimizer*** and the ***OptimizationResult*** it returns) along:

- Numerical correctness against problems with a known minimum: every
local method is driven to the minimiser of a well-conditioned
...
- Performance: wall-clock timing of several methods on the same
problem, the evaluation-count cost of a numerical gradient against an
analytical one, and the Hessian-vector path against forming the full
Hessian.
...
--------------------
Optimisation, unlike differentiation, returns an approximate minimiser, not
...
from ripples.optimization import (
    minimizer
)
...
def test_block(test_name: str, description: str) -> None:
    ...
    `test_name`, mapped A -> 1 ... S -> 19), <m> is the position of this
    ...
    test_name : str
        ...
        "L.invalid.unknown_method". The same string is handed to the
...
def check_allclose(
    ...
    # mirroring the relative-error formula of OptimizationResult.
...
# TEST PROBLEMS
#
# A small library of objectives with known minima, shared across the sections
# below. Each comes with an analytical gradient (and, where a section needs
# one, an analytical Hessian / Hessian-vector product) so a test can choose
# between feeding the derivative to the solver or letting the module build it
...
# can reach the minimiser quickly, which makes it the right vehicle for the
# "does every method work at all" section.
QUADRATIC_TARGET = np.array([1.0, -2.0, 0.5])


def quadratic_bowl(params: np.ndarray) -> float:
    """f(x) = 0.5 * sum((x - target)^2); minimiser at `QUADRATIC_TARGET`."""
...
def rosenbrock(params: np.ndarray) -> float:
    ...
    valley whose unique minimiser is the all-ones vector with f = 0.
    """
...
def booth(params: np.ndarray) -> float:
    """Booth's function; smooth, convex, minimiser at (1, 3) with f = 0."""
    x = np.asarray(params, dtype=float)
...
def rastrigin(params: np.ndarray) -> float:
    ...
    of local minima, with the single global minimiser at the origin (f = 0).
...
# SECTION A - EVERY LOCAL METHOD REACHES THE MINIMUM
#
# The most basic guarantee the module makes: pick a function with a known
# minimiser and check that each local algorithm gets there. The quadratic bowl
# is perfectly conditioned (identity Hessian) so the result depends on the
# optimiser machinery, not on the difficulty of the landscape. The two
# first-order methods, which the module documents as weak general-purpose
# solvers, are started near the optimum and judged on a relaxed band, exactly
# the regime in which they are meant to be used.



def section_a_local_method_correctness() -> None:
    """
    ...
    section_banner("SECTION A - Every local method reaches the minimum")
    ...
    for (
        ...
        test_block(
            test_name=f"A.reaches_minimum :: {method}",
            description=(
                f"verify {method} drives the quadratic bowl to its known "
                f"minimiser {QUADRATIC_TARGET.tolist()}, running it from "
        ...
        check_allclose(
            ...
            interpretation=f"{method} located the minimiser.",
        )



# SECTION B - CLASSIC HARD TEST FUNCTIONS
#
# A perfectly conditioned quadratic flatters every method. The real test of
# the strong methods is a curved, ill-scaled valley. Rosenbrock is the
# standard one; Booth adds a second, differently shaped convex problem solved
# with a finite-difference gradient to exercise that path on a real objective.



def section_b_classic_test_functions() -> None:
    """
    ...
    analytical gradient, requiring both the minimiser (1, 1) and a success
    verdict. Then solve Booth from the origin with a numerical gradient.
    """

    section_banner("SECTION B - Classic hard test functions")

    rosenbrock_minimiser = np.array([1.0, 1.0])
    ...
    for method in strong_methods:
        test_block(
            test_name=f"B.rosenbrock :: {method}",
            description=(
                f"verify {method} converges to the Rosenbrock minimiser "
        ...
        print(f"  RESULT      : final_cost = "
              ...
              f"iterations = {result.iteration_number}, "
              f"success = {result.success}")
        reached_minimiser = bool(np.allclose(
            np.asarray(result.final_params), rosenbrock_minimiser,
            rtol=1e-3, atol=1e-4,
        ))
        check_truth(
            test_name=f"B.rosenbrock :: {method}",
            condition=reached_minimiser and result.success,
            message_on_pass=(
    ...
    test_block(
        test_name="B.booth_numerical_gradient",
        description=(
            "verify lbfgs solves Booth's function to its minimiser (1, 3) "
    ...
    check_allclose(
        ...
        interpretation="the numerical-gradient path reaches Booth's minimiser.",
    )



# SECTION C - ANALYTICAL VS NUMERICAL GRADIENT
#
# The module promises the same call shape whether or not an analytical
# gradient is available. Here we run the same problem both ways and check the
# two optima agree and that the result records which path was taken.



def section_c_analytical_vs_numerical_gradient() -> None:
    """
    Minimise Rosenbrock twice with bfgs, once with the analytical gradient and
    once with the finite-difference fallback, and check the two minimisers
    agree and that `analytical_gradient_provided` distinguishes them.
    """

    section_banner("SECTION C - Analytical vs numerical gradient")
    ...
    test_block(
        test_name="C.same_optimum",
        description=(
            "verify the analytical-gradient and numerical-gradient runs reach "
            "the same Rosenbrock minimiser, solving once each and comparing "
    ...
    test_block(
        test_name="C.flag_reflects_source",
        description=(
...
# SECTION D - SECOND-ORDER INFORMATION
#
# The trust-region and Newton-CG methods can take curvature from a full
# Hessian, from a Hessian-vector product, or build it numerically. We feed an
...
def section_d_second_order_information() -> None:
    """
    Solve the SPD quadratic (unique minimiser at the origin) with trust_ncg
    ...
    section_banner("SECTION D - Second-order information (Hessian and HVP)")

    start = _SPD_RNG.standard_normal(4)

    # Hessian path.
    test_block(
        test_name="D.analytical_hessian",
        description=(
            "verify trust_ncg with an analytical Hessian drives the SPD "
            "quadratic to the origin and sets analytical_hessian_provided, "
            "solving from a random start and checking the minimiser and the "
    ...
    test_block(
        test_name="D.hvp_takes_precedence",
        description=(
    ...
    both_result = check_warns_returning(
        test_name="D.both_hessian_and_hvp_emits_warning",
...
# SECTION E - STARTING-POINT INDEPENDENCE
#
# A convex problem has a single minimiser, so a correct local method must
# reach it regardless of where it starts. Disagreement between starts would
# point to a path-dependent bug (a mis-scaled step, a bad reset) rather than
# to the mathematics.



def section_e_starting_point_independence() -> None:
    """
    Solve the convex quadratic bowl from several very different starting
    points with lbfgs and check every run lands on the same minimiser.
    """

    section_banner("SECTION E - Starting-point independence")
    ...
    for start in starting_points:
        test_block(
            test_name=f"E.convex_start :: {start.tolist()}",
            description=(
                f"verify lbfgs reaches the quadratic bowl minimiser "
        ...
        check_allclose(
            ...
            interpretation="the start did not change the minimiser.",
        )



# SECTION F - GLOBAL SEARCH: DIRECT
#
...
# minimiser sits in the interior and check it is located, that no gradient was
# ever evaluated, and that the bounded / global flags are set.



def section_f_global_direct() -> None:
    """
    Minimise a bounded bowl with the minimiser in the box interior using
    DIRECT, and check the located point, the zero gradient count, and the
    is_bounded / is_global_method flags.
    """

    section_banner("SECTION F - Global search: DIRECT")

    interior_minimiser = np.array([0.3, -0.4])

    def bounded_bowl(params: np.ndarray) -> float:
        return float(np.sum((np.asarray(params) - interior_minimiser) ** 2))
    ...
    test_block(
        test_name="F.locates_interior_minimum",
        description=(
            "verify DIRECT locates the interior minimiser (0.3, -0.4) of a "
...
# SECTION G - GLOBAL SEARCH: DUAL ANNEALING
#
...
def section_g_global_annealing() -> None:
    """
    ...
    section_banner("SECTION G - Global search: dual annealing")
    ...
    test_block(
        test_name="G.escapes_local_minima",
        description=(
    ...
    test_block(
        test_name="G.seed_reproducibility",
        description=(
...
# SECTION H - BOX BOUNDS
#
# Supplying bounds to a local method engages the augmented-Lagrangian outer
# loop. The cleanest test pushes the unconstrained optimum outside the box, so
# the solver has to settle on the boundary rather than at the free minimum.



def section_h_box_bounds() -> None:
    """
    ...
    section_banner("SECTION H - Box bounds via augmented Lagrangian")
    ...
    test_block(
        test_name="H.optimum_on_the_box",
        description=(
    ...
    test_block(
        test_name="H.bounded_not_constrained",
        description=(
...
# SECTION I - EQUALITY CONSTRAINT
#
# A single equality constraint turns the problem into "find the closest
# feasible point". We use a geometry with a hand-checkable answer (the foot of
# the perpendicular from a point to a line) and confirm both the answer and
# that the constraint is actually satisfied at the solution.



def section_i_equality_constraint() -> None:
    """
    ...
    section_banner("SECTION I - Equality constraint")
    ...
    test_block(
        test_name="I.foot_of_perpendicular",
        description=(
    ...
    test_block(
        test_name="I.constraint_satisfied_and_counted",
        description=(
...
# SECTION J - INEQUALITY CONSTRAINT
#
# Inequalities are written c(x) >= 0. We place the unconstrained optimum
# (the origin) outside the feasible half-plane x + y >= 2, so the constrained
# optimum is forced onto the active boundary at the hand-computable point
# (1, 1).



def section_j_inequality_constraint() -> None:
    """
    ...
    section_banner("SECTION J - Inequality constraint")
    ...
    test_block(
        test_name="J.optimum_on_active_boundary",
        description=(
    ...
    test_block(
        test_name="J.feasible_at_solution",
        description=(
...
# SECTION K - OPTIMIZATIONRESULT API SURFACE
#
...
# single known run. The run is trust_ncg on 2-D Rosenbrock, whose minimiser
# (1, 1) and success verdict are documented in the minimizer examples.
...
def section_k_result_api_surface() -> None:
    ...
    section_banner("SECTION K - OptimizationResult API surface")
    ...
    test_block(
        test_name="K.convenience_properties",
        description=(
    ...
    test_block(
        test_name="K.as_float_scalar_ok",
        description=(
...
# SECTION L - PARAMETER VALIDATION
#
...
def section_l_parameter_validation() -> None:
    """
    ...
    section_banner("SECTION L - Parameter validation")
    ...
    invalid_calls = [
        ...
        (
            "L.invalid.missing_initial_params",
        ...
        (
            "L.invalid.unknown_method_param_key",
        ...
        (
            "L.invalid.unknown_tolerance_key",
        ...
        (
            "L.invalid.max_iters_bad_string",
        ...
        (
            "L.invalid.verbose_frequency_zero",
        ...
        (
            "L.invalid.verbose_frequency_bad_string",
        ...
        (
            "L.invalid.bounds_not_unpackable",
        ...
        (
            "L.invalid.required_bound_is_none",
        ...
        (
            "L.invalid.constraint_missing_keys",
        ...
        (
            "L.invalid.constraint_fun_not_callable",
...
# SECTION M - WARNING EMISSION
#
# A global method does not use a starting point, so passing one is harmless but
# almost certainly a mistake on the caller's part. The module warns rather than
# errors; we confirm the warning is actually raised.



def section_m_warning_emission() -> None:
    """
    ...
    section_banner("SECTION M - Warning emission")

    test_block(
        test_name="M.initial_params_for_global_method",
        description=(
...
# SECTION N - CALLBACK AND TRAJECTORY
#
# The per-iteration callback is how a caller plots a convergence curve or
# stops early on a custom criterion. We record the cost at each iteration and
# check the callback fired and that the cost fell from start to finish.



def section_n_callback_and_trajectory() -> None:
    """
    ...
    section_banner("SECTION N - Callback and trajectory")
    ...
    test_block(
        test_name="N.callback_fires_and_cost_falls",
        description=(
...
# SECTION O - CUSTOM SETTINGS ARE HONOURED
#
# Tolerances, per-method parameters, the numerical-gradient stencil width and
# an explicit iteration cap are all user-facing knobs. We confirm each one is
# both reflected on the result and compatible with convergence.



def section_o_custom_settings() -> None:
    """
    ...
    the run still reaches the minimiser.
    """

    section_banner("SECTION O - Custom settings are honoured")

    # Custom tolerance.
    test_block(
        test_name="O.custom_tolerance",
        description=(
            ...
            "and checking both the recorded value and the minimiser; the "
    ...
    test_block(
        test_name="O.gradient_point_number",
        description=(
            ...
            "analytical_gradient_provided flag and the minimiser; the wider "
    ...
    test_block(
        test_name="O.explicit_max_iters",
        description=(
...
# SECTION P - HIGH-DIMENSIONAL SCALING
#
# A limited-memory method should handle a problem far larger than the toy
# cases above. We solve a 50-variable well-conditioned quadratic with L-BFGS
# and check it reaches the minimiser entry by entry.



def section_p_high_dimensional() -> None:
    """
    Solve a 50-D well-conditioned quadratic with L-BFGS and the analytical
    gradient, checking the minimiser is recovered, the run reports success,
    and the dimensionality is reported as 50.
    """

    section_banner("SECTION P - High-dimensional scaling")
    ...
    test_block(
        test_name="P.fifty_variable_quadratic",
        description=(
            "verify L-BFGS recovers the minimiser of a 50-variable quadratic "
    ...
    check_allclose(
        ...
        relative_tolerance=1e-4, absolute_tolerance=1e-5,
        interpretation="the 50-D minimiser is recovered to a few digits.",
    )



# SECTION Q - METHOD-SPEED BENCHMARK
#
# Pure performance information: how do the strong methods compare in wall-clock
# time and in evaluation counts on the same problem? This is reported without a
# hard threshold, since the right answer depends on the machine.



def section_q_method_speed_benchmark() -> None:
    """
    ...
    section_banner("SECTION Q - Method-speed benchmark")
    ...
    test_block(
        test_name="Q.method_speed",
        description=(
            "measure the wall-clock time and evaluation counts of each strong "
...
# SECTION R - GRADIENT-COST BENCHMARK
#
...
def section_r_gradient_cost_benchmark() -> None:
    """
    ...
    section_banner("SECTION R - Gradient-cost benchmark")
...
def section_s_hvp_vs_hessian_benchmark() -> None:
    """
    ...
    test_block(
        test_name="S.hvp_vs_full_hessian",
        description=(
...
# MAIN ENTRY POINT
#
# `run()` is the single orchestration point shared by every way of launching
# the suite: the `ripples.test()` dispatcher, the
# `python -m ripples.optimization._test` command line, and the pytest bridge
# below all come through here, so the output and the verdicts are identical no
# matter how the suite is started.
...
def run(include_benchmarks: bool = True) -> Reporter:
    ...
    benchmark_sections = {
        section_q_method_speed_benchmark,
        section_r_gradient_cost_benchmark,
        section_s_hvp_vs_hessian_benchmark,
    }
    ordered_sections = [
        section_a_local_method_correctness,
        section_b_classic_test_functions,
        section_c_analytical_vs_numerical_gradient,
        section_d_second_order_information,
        section_e_starting_point_independence,
        section_f_global_direct,
        section_g_global_annealing,
        section_h_box_bounds,
        section_i_equality_constraint,
        section_j_inequality_constraint,
        section_k_result_api_surface,
        section_l_parameter_validation,
        section_m_warning_emission,
        section_n_callback_and_trajectory,
        section_o_custom_settings,
        section_p_high_dimensional,
        section_q_method_speed_benchmark,
        section_r_gradient_cost_benchmark,
        section_s_hvp_vs_hessian_benchmark,
    ]
